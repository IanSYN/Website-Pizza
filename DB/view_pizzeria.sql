...
AS 
	(SELECT  VP.idCommande, nomProduit, quantiteProduit, quantitePizza, VP.idClient, prenomClient, prixTotalCommande
	FROM `VPanier` VP
	INNER JOIN `VPanierPizza` VPP ON VPP.idCommande = VP.idCommande);
